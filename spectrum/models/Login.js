					...
					if(err) {
				     ...
				     if(Login.Linked === 'Linked'){
							   if(row.password === Login.password && (row.username === Login.Mail)){
								...
								if(row.password === 'spectrum' && row.username === Login.Mail){
									console.log('please set your password!!!');

												    var obj123 = {response:'6',message:'You have already logged in from social media so please set your password'};
													res(obj123);
								}
								else if(row.password === Login.password && (row.username === Login.Mail)){
									   console.log('Login success!!');
