...
var Student={
		...
		getAllTasks:function(callback){
			...
			*/
			...
			updateTask:function(mail,Student,callback){
			  ...
			  if(err) {
			   ...
			   console.log("The file was saved!");
			    ...
			    return  db.query("update personalinfo set Height=?,Weight=?,DOB=?,Blood_Group=?,AddedTime=?,profilepic=? where username=?",[Student.RelativeName,Student.RelativeType,Student.Height,Student.Weight,Student.DOB,Student.Blood_Group, Student.AddedTime,path1,mail],callback);
			},
			/*
 ...
 if(err) {
   ...
   console.log("The file was saved!");
    ...
    var q = db.query("Insert into personalinfo values(?,?,?,?,?,?,?,?,?)",[Student.Mail,Student.Name,Student.Gender,Student.Height,Student.Weight,Student.DOB,Student.Blood_Group, Student.AddedTime,path1],function(err,row,fields){
    	if(err){
    		console.log(err);
