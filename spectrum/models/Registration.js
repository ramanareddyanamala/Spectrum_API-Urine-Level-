					...
					if(err) {
						    ...
						    res(err);   
								 ...
								 res(r);
					        	   ...
					        	   var query12 = db.query("select mobile from registration where mobile=?",[Add.Mail],function(err,row,fields){
									...
									//	var timeStampInMs = window.performance && window.performance.now && window.performance.timing && window.performance.timing.navigationStart ? window.performance.now() + window.performance.timing.navigationStart : Date.now();
										...
										var que = db.query("Insert into registration(mobile,pin,registerd_time,latitude,longitude) values(?,?,?,?,?)",[Add.Mail,text,Add.RegTime,Add.lat,Add.long],function(err,row,fields){
										if(err) {
											    console.log('Error occured!');
											    var r = {response:'0'};
											    res(r);   
									...
									var q2 = db.query("update registration set pin=?,registerd_time=? where mobile=?",[text12,Add.RegTime,Add.Mail],function(err,row,fields){
										if(err) {
										    console.log('Error occured!');
										    var e = {reponse:'0'};
										    res(err);   
			...
			if(validator.isEmail(Add.Mail)) {
				var query2 = db.query("select username from login where username=?",[Add.Mail],function(err, row, fields) {
					...
					if(err) {
						...
						var query12 = db.query("select mail from registration where  mail=?",[Add.Mail],function(err,row,fields){
			        	     console.log(row.length);
			        	     if(err){
			        	    	 res(err);
			        	     }
						 ...
						 // var timeStampInMs = window.performance && window.performance.now && window.performance.timing && window.performance.timing.navigationStart ? window.performance.now() + window.performance.timing.navigationStart : Date.now();
						  //console.log(timeStampInMs, Date.now());
						  var q = db.query("Insert into registration(mail,pin,registerd_time,latitude,longitude) values(?,?,?,?,?)",[Add.Mail,text1,Add.RegTime,Add.lat,Add.long],function(err,row,fields){
							  if(err) {
								    console.log('Error occured!');
								    var er ={response:'0'}; 
								    res(err);   
									    													...
									    													<table width="100%" border="0" cellspacing="0" cellpadding="0" style=" color: #222121; font-family: Times New Roman, Times, serif; font-size: 20px; line-height: 16px;">
									    														<tr>
									    															<td style="padding:50px 15px 20px; width = 100%">
									    																...
									    																<pre style="padding:0; font-family: Times New Roman, Times, serif;"><strong>Please note:</strong> If you have not attempted to register with Palla Restaurant, please ignore this email.</pre>
									    																<br>
						  ...
						  //var timeStampInMs1 = window.performance && window.performance.now && window.performance.timing && window.performance.timing.navigationStart ? window.performance.now() + window.performance.timing.navigationStart : Date.now();
						  //console.log(timeStampInMs1, Date.now());
						  var q3 = db.query("update registration set pin = ?, registerd_time =? where mail = ?",[text12,Add.RegTime,Add.Mail],function(err,row,fields){
							  if(err) {
								    console.log('Error occured!');
								    var c = {response:'0'};
								    res(err);   
