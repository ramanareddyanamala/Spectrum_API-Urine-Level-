		...
		forgot:function(Forgot,res,err,callback){
			if(!validator.isEmail(Forgot.Mail)){
				...
				var query1 = db.query("select username from login where username=?",[Forgot.Mail],function(err, row, fields) {
				    ...
				    service: 'gmail',
				    auth: {
				    user: 'ramanareddy0012@gmail.com',
				    pass: '09091234'
				  }
				  });
				 var mailOptions = {
				  from: 'ramanareddy0012@gmail.com',
				  to: Forgot.Mail,
				...
				transporter.sendMail(mailOptions, function(error, info){
				  ...
				  db.query("Update registration set pin = ?, registerd_time=? where mail = ?",[text1,Forgot.RegTime,Forgot.Mail],callback);
